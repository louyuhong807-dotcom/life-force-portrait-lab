...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  if (!apiKey) return response.status(503).json({ error: "AI 清理服务尚未配置", code: "not_configured" });

  try {
    return response.status(200).json(await cleanupPortraitBackground((request.body ?? {}) as Record<string, unknown>, apiKey));
  } catch (error) {
    if (error instanceof AiImageError) return response.status(error.status).json({ error: error.message, code: error.code });
    if (error instanceof DOMException && error.name === "TimeoutError") {
      return response.status(504).json({ error: "清理时间较长，请稍后重试", code: "timeout" });
    }
    return response.status(500).json({ error: "AI 清理暂时不可用", code: "unknown" });
  }
}
